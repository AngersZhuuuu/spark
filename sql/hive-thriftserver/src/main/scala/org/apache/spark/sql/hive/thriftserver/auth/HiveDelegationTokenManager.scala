...
object HiveDelegationTokenManager {
  ...
  val DELEGATION_TOKEN_STORE_ZK_CONNECT_STR = "hive.cluster.delegation.token.store.zookeeper.connectString"
  ...
  val DELEGATION_TOKEN_STORE_ZK_CONNECT_TIMEOUTMILLIS = "hive.cluster.delegation.token.store.zookeeper.connectTimeoutMillis"
...
class HiveDelegationTokenManager {
  ...
  @throws[IOException]
  def startDelegationTokenSecretManager(conf: HiveConf, hms: Any, smode: Server.ServerMode): Unit = {
    val secretKeyInterval = conf.getLong(DELEGATION_KEY_UPDATE_INTERVAL_KEY, DELEGATION_KEY_UPDATE_INTERVAL_DEFAULT)
    val tokenMaxLifetime = conf.getLong(DELEGATION_TOKEN_MAX_LIFETIME_KEY, DELEGATION_TOKEN_MAX_LIFETIME_DEFAULT)
    val tokenRenewInterval = conf.getLong(DELEGATION_TOKEN_RENEW_INTERVAL_KEY, DELEGATION_TOKEN_RENEW_INTERVAL_DEFAULT)
    val tokenGcInterval = conf.getLong(DELEGATION_TOKEN_GC_INTERVAL, DELEGATION_TOKEN_GC_INTERVAL_DEFAULT)
  ...
  @throws[IOException]
  def renewDelegationToken(tokenStrForm: String): Long = secretManager.renewDelegationToken(tokenStrForm)
  ...
  private def getTokenStore(conf: Configuration): DelegationTokenStore = {
    ...
    try {
      val storeClass = Class.forName(tokenStoreClassName).asSubclass(classOf[DelegationTokenStore])
